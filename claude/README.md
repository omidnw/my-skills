...
Portable **Agent Skills** plus always-loaded **rules** for [Claude Code](https://code.claude.com). Copy this folder into any project (or into `~/.claude/` for global use) and Claude Code discovers the skills automatically and loads the rules on every session.
...
```bash
cp -R claude/.claude .            # skills + rules
cp claude/CLAUDE.md .             # optional: project instructions
```
...
cp -R claude/.claude/rules ~/.claude/rules/
...
```
.claude/
├── skills/      9 Agent Skills (SKILL.md + references/ + scripts/)
└── rules/       always-loaded rules: language, safety, agent-behavior, engineering-preferences
CLAUDE.md        project instruction file (always-English rule + pointers)
```

Rules in `.claude/rules/` without a `paths` frontmatter field are loaded unconditionally at launch.
