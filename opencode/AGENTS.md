...
OpenCode loads rules from `AGENTS.md` files (project root, merged with `~/.config/opencode/AGENTS.md`). Skills live in `.opencode/skills/` and load on demand.
...
Skills live in `.opencode/skills/` (9 portable skills, standard `SKILL.md` format; the ZCode-only `context-compact` and `session-handoff` ship only in `zcode/`). See `README.md` for the "what it does / why it was created" table.