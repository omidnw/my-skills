...
Portable **Agent Skills** plus **rules** for [OpenCode](https://opencode.ai). OpenCode loads rules from `AGENTS.md` (project root, merged with `~/.config/opencode/AGENTS.md`) and discovers skills from `.opencode/skills/` on demand.
...
```bash
cp -R opencode/.opencode .        # skills
cp opencode/AGENTS.md .           # rules + instructions
```

**Global** — copy into your config directory:
...
For project paths, OpenCode walks up from the working directory to the git root and picks up skill folders along the way. Skills are exposed through a native skill tool and their full content loads only when needed.
